...
function createSectionElement(section, sectionName) {
  ...
  if (isCard) {
    ...
    if (section.textSettings) {
      ...
      const writingMode = section.textSettings.writingMode;
      element.style.writingMode = writingMode.replace('-upright', '');
      element.style.textOrientation = writingMode.includes('-upright')
        ? 'upright'
        : 'mixed';
    }
...
export function generateHTML() {
  ...
  pre.textContent = `<div style="all: unset">${card.outerHTML}</div>`;
